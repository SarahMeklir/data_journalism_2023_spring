...
**Answer** 55,632 rows (observations) and 7 columns (variables)
...
**Answer** "id" is characters and "seats" is double (chr, dbl respectively)
...
**Answer** umd_courses %>% selects the umd_courses dataframe to execute the following actions on. I think the %>% might be sorting high to low, but I'm not sure (turns out that means "and then," like a connector, so disregard). group_by(term) %>% selects all the classes in one month of one year to count. It also sorts the groups from oldest year and month to most recent year and month. summarise( ) designates what to output. count_classes = n() I imagine counts all the classes in each block grouped by the previous code, so each time frame's total number of classes offered.
...
**Answer** This one has the class number in order from highest to lowest, and each total's corresponding time/year/term/whatever
...
**Answer** This one includes department in the output, and sorts alphabetically by department within each term, and the terms are listed from earliest to most recent.
...
Compared to the previous codeblock, what has changed and how did that affect the result? 
**Answer** This one shows seat totals instead of department, and it's sorted by seat total in descending order.
...
**Answer** It's still sorted with the most to least seats, but this has more columns and math is involved, which is fun.
...
**Answer** This one is still sorted by total seats in decending order, but rather than being separated into terms, it's separated into departments. Also, the math has remained.
...
1.  How has this ranking of most seats based on department changed over time?

2.  How much impact does class popularity (so amount of classes offered and amount of open seats at a certain date) have on the seats and classes offered the next term and next years? How has that impacted the this seat-based ranking of departments over time?

3.  What department has the most courses with zero seats available? Is that a reliable way to interpret classes as popular or would that be confusing if a class is just commonly required?
