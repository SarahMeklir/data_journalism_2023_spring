...
**Answer:** The date shifted from month/day/year with no extra characters (2 instead of 02, 22 instead of 2022) to year-month-day with all the characters (2022 and 02), and it went from chr type to date type. It's kind of a permanent replacement, though, so gotta be careful about it because otherwise you have to reload the data.
...
**Answer** It's only displaying the first of each month, so I think it's consolidating all the month's individual days' total expenditures to one day.
...
**Answer** It's making a new column (total_votes) counting up all the votes for all the candidates, and another new column (pct_moore) seeing what percent of the total votes Wes Moore received by dividing his votes by the newly created total votes count. It will show up as a decimal because it's just dividing and not multiplying by 100. 
...
**Answer** The output organizes the data by the percent of the total vote Wes Moore recieved, from highest percentage to lowest. It's also now in actual fraction form because it got multiplied by 100.
...
**Answer** I don't think it's more useful, but it is interesting. The lowest percentage of the vote he got was 22%, which still seems like a hefty chunk.
...
**Answer** str_to_lower changes a string to lowercase, and str_to_title changes the first letter of each word to be uppercase.
...
**Answer** First it's naming the function to specify that the data it produces is associated with states. It's finding states and changing them to an output that is more consistent and convenient to analyze the data. 
...
**Answer** If my understanding of it is right, it does make sense. It's displaying the total money from each state we previously specified. 
...
1. I'd try to add commas to that last data table from Maryland expenditures so it is easier to read quickly at a glance. 

2. I'd be interested to see how many course titles mention social media since 2017 and how that number has changed

3.  I'd like to see which teacher has taught in the most departments, but that may be difficult because if two or more instructors share a name it's hard to fix that problem with the data we have.
