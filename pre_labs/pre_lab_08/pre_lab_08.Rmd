...
**Answer** 3,822 rows and 4 columns
...
**Answer** Line 1: Creates a new dataframe designated by month from the Baltimore County 911 calls
Line 2: Creates a new column called month which takes the date from the existing date column, finds the month, and transforms it into just the month in a new column. I think the "true" means as long as there was one call that month.
Line 3: groups by month so we can find the total calls per month
Line 4: counts up the total number of calls per month
Line 5: reorders the months so the one with the most calls appears first and descends in value from there
Line 6: displays the table we just created
...
**Answer** I see a blank grey rectangle almost the width of the window.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** There's a bar chart now! It's got "month" on the x-axis and "count" on the y-axis. It shows how many calls there were per month. All the bars are dark grey, the background is a gridded lighter grey, and the outline of all of it is white.
...
**Answer** The graph is arranged in ascending order now. This time, reorder was used in the code so the graph knew to order by month with least to most calls.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** All that changed was adding coord_flip() at the bottom so it was oriented horizontally rather than vertically.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The grid is no longer grey in the background, it's all white. It adds contrast with the data and helps the eye not be distracted at a first glance.
...
**Answer** It's got a blue background now! And darker text on the sides and bottom. Although the y-axis label is overlapping with the month labels now. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** This is really cool! It seems pretty intuitive, and I like how easily you can manipulate the look of the chart. This added a title, simplified the axis labels, and added a source at the bottom.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The table is grouped by date overall, not by month, so there are a lot more data points (366) and it's counting the total calls per day.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** That looks like a heart monitor for someone whose heart is beating dangerously fast. There's a serious high point in the beginning of October and overall it's very frenzied.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Oh no, that gave a little too much detail. All the labels on the x-axis are overlapping and totally incomprehensible.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** That's so cool! The labels are tilted now. It's still really crammed but it's legible.
```{r}
baltco_911_calls_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_calls)) + 
