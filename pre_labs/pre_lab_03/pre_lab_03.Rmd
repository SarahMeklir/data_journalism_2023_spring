...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** Now the zip code column appears as a character column. 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** I was told to skip this.

```{r}
glimpse(payments)
```

Things that should be characters -- like agency name, vendor name -- are characters (chr). Things that should be numbers (dbl) -- like amount and fiscal year -- are numbers. We've seen before that sometimes dateks aren't defined as date datatypes by R - we can fix that using `lubridate`.
...
**Answer**
Thousands of zip codes don't have five characters. I think whoever was in charge of the data accidentally dropped zeros in the front or back of the number and that left it with four digits.
```{r}
payments %>%
  group_by(`Vendor Zip`) %>%
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer** N/As, L, and Contract
...
**Answer** Lots of values from 300 all the way to the twenty thousands, but few small values.
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** It adds a new column shortening 9 digit zip codes to 5 digits so now we have a column with all five-digit zips.
...
**Answer** I'm not sure how to get it to count each donation. I get the concept of filtering what I need, I think, but I'm not sure how to arrange or count it the way I'm supposed to.
