...
**Answer** It simplifies it so the repeats are ignored/consolidated into one instance.
...
**Answer** There are some words and some letters or weird characters that I wouldn't call meaningful.
...
**Answer**
It's more interesting for sure. There's a lot of health talk.
```{r}
unique_words %>%
...
**Answer**
Covid is big in 2021, projects and community in 2022, and women are coming out strong in '23 so far.
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer** just... just so much covid. That's crazy. Health and response plans and pandemic stuff is very prevalent
...
**Task** Run the following code and describe the results.
**Answer**
Less pandemic, more maryland-centric
```{r}
releases %>%
...
**Task** Run the following code and describe the results.
**Answer** Oral health and offshore wind making their debut! Exciting!
...
**Answer**
They're pretty similar. Critical could be neutral when it means something is important, but it's listed as negative.
```{r}
