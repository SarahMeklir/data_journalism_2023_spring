...
**Answer** It shows the state, who got the majority of the vote in 2016 and by how much, the percentage of businesses getting the loans, and the amount per employee.
```{r}
...
**Answer** There's a positive relationship. Not a huge one, not super closely clumped datapoints, but there's for sure correlation.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** There's less of a positive relationship. 
```{r}
...
**Answer** It gets rid of the state name and the vote, it simplifies the output.
...
**Answer** I see some words I didn't really want to see again after stats, but it's helpful and gives us valuable information. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer**Its p-value is higher and its correlation is lower.
```{r}
...
**Answer** Average loans given and amount per employee showing Trump states got more loans and more money in them
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** This tells us whether the difference above was meaningful. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The p-value is less than .05, but it doesn't mean there was a conscious effort by Trump to approve more loans in states that supported him.
```{r}
