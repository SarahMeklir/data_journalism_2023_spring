...
**Answer** Multipolygon is something new. 
...
**Answer** Zipcodes, cities, geometry (?), and the area of the shapes.
...
**Answer**
It's a very detailed map of Maryland with lots of little territories outlined within.
```{r}
md_zips %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It installed a package, there's not a ton to report
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It's loading! That's so much information. County names are weird. Here are some fun ones: Beaver! Menominee (sounds like that mana mana muppet song)! Ontonagon (just cool). Crisp. Kittitas. Yell County (probably loud)
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It's counties including the Chesapeake Bay.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It's counties AND zipcodes with the counties overlapping waterways but zipcodes avoiding them. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
Foreclosure data for march 2023 in each zipcode
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Added the table we just made to the larger table
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It's really cool! The counties with more foreclosures are lighter, which is inverted to the usual style I tend to see. Also, why is the color theme for blue called "magma"?
```{r}
ggplot() +
...
**Answer**
No output! Just running some code. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Rows to columns and columns to rows! Limits data to just MD counties. Also it's incorporating population data so we can make our map more accurate and not just a population map
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We need a geometry column! So we're grabbing the geometry column by adding geometry = TRUE. Sweet.
```{r}
md_county_population <- get_acs(geography = "county",
              variables = c(population = "B01001_001"),
              year = 2021,
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Need to clean up the  ", Maryland" part of the population table so it can sync up with the notices county column
```{r}
md_county_population
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We made everything line up so we can now join the two together.
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Joining things! Everything seems like it lined up. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
HoCo and MoCo are very low. Theres an incredible discrepancy in rates from the highest to the lowest.
```{r}
md_pop_with_foreclosures <- md_county_population %>%
  left_join(md_county_notices, by='county') %>% 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Hooray for rate by population maps! That's really significant in Charles County.
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It's more helpful because we can distinguish things easier. However, I'm a fan of blue so I'm a little sad it's gone. 
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
...
**Answer**
Even more distinct differences between counties. The western shore has a lot of foreclosures, but none of them near Charles County
```{r}
ggplot() +
  geom_sf(data=md_pop_with_foreclosures, aes(fill=rate)) +
  theme_minimal() +
